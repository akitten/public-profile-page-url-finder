...
module.exports = function profile(url, html, next) {
  ...
  for(i=0; i < titleLinks.length; i++) {
    if (titleLinks[i].children[0].attribs) {
      link = titleLinks[i].children[0].attribs.href;
    }
    
    if(!link) {
       if (titleLinks[i].children[1]) {
        link = titleLinks[i].children[1].attribs.href;
      }
    }
    if( link ) {
      if( link.match(/linkedin.com\/in/) || ( link.match(/linkedin.com\/pub/) && !link.match(/linkedin.com\/pub\/dir/)) &&
        !link.match(/https:\/\/any-na.www.linkedin.com/)) {
